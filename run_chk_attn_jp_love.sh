...
TRAINED_MODEL=./results/net_trained_love.pth
TSV_FILE=./data/train_love.tsv
...
function run_all() {
    for i in `seq 2001`
    do
    poetry run python check_attention.py --batch_size ${BATCH_SIZE} --text_length ${TEXT_LENGTH} --index $i --save_raw_attn ./results/attention_love/sentence_$i  ${MECAB_OPT}   ${CONF_FILE}  ${TRAINED_MODEL}  ${TSV_FILE}  ${VOCAB_FILE}
    done
}
